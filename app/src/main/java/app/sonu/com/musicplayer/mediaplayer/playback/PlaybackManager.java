...
import app.sonu.com.musicplayer.mediaplayer.QueueManager;
...
public class PlaybackManager implements Playback.Callback {
    ...
    public void handlePlayRequest(){
        ...
        MediaSessionCompat.QueueItem currentSong = mQueueManager.getCurrentSong();
        if (currentSong != null) {
            if (mPlayback.play(currentSong)){
    ...
    public void updatePlaybackState(String error) {
        ...
        // put current queue item index in extras
        extras.putInt(Playback.PLAYBACK_STATE_EXTRA_CURRENT_QUEUE_INDEX,
                mQueueManager.getmCurrentQueueIndex());

        MediaSessionCompat.QueueItem currentSong = mQueueManager.getCurrentSong();
        if (currentSong != null) {
            // Set the activeQueueItemId if the current index is valid.
            stateBuilder.setActiveQueueItemId(currentSong.getQueueId());

            // if song is in favorites, put in extras
            extras.putBoolean(PlaylistsSource.PLAYBACK_STATE_EXTRA_IS_IN_FAVORITES,
                    mPlaylistsManager.isSongIdInFavourites(QueueHelper.getSongIdOf(currentSong)));
        }
    ...
    private class MediaSessionCallback extends MediaSessionCompat.Callback {
        ...
        public void onPlayFromMediaId(String mediaId, Bundle extras) {
            ...
            mQueueManager.setQueueFromSongMediaId(mediaId);
            if (mShuffleMode==1) {
        ...
        public void onSkipToQueueItem(long id) {
            ...
            if (mQueueManager.getCurrentSong().getQueueId() != id) {
            ...
            if (setShuffleMode(mode)) {
                if (mQueueManager.getCurrentSong() != null) {
                    if (enabled) {
                        ...
                        mQueueManager.setQueueFromSongMediaId(
                                mQueueManager.getCurrentSongMediaId(), true);
                    }
                }
